...
class IPythonNB(BaseReader):
    ...
    def read(self, filepath):
        ...
        try:        
            ...
            metadata = json.load(ipynb_file)['worksheets'][0]['cells'][0]['input']
            open(metadata_filepath, 'w').write(''.join(metadata[1:])+'\n')
        except:
            print('Error generating cell meta')
        ...
        if BeautifulSoup:
            ...
            for i in soup.findAll("div", {"class" : "input"}):
                if len(i.find_all(text='#ignore')) > 0:
                    i.extract()
        else:
            soup = content
